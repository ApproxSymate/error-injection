...
int fill_bitflip(int x, int y) {
  int width = bitflip_int(WIDTH);
  int height = bitflip_int(HEIGHT);
#ifdef AGGRESSIV
  ...
  while (1) {
    ...
    int x1 = bitflip_int(x);
    int x2 = bitflip_int(x);
    ...
    int inScanLine = 0;
    int i;
    for (i = bitflip_int(x1); i <= bitflip_int(x2); i++) { // find scan-lines above this one
    ...
    inScanLine = 0;
    for (i = bitflip_int(x1); i <= bitflip_int(x2); i++) { // find scan-lines below this one
