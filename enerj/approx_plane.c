...
int texture1_bitflip(
  ...
  if (bitflip_int(texture) == 1) {
    ...
    if (v == 0) {
      col = bitflip_int((255 << 24) | b);
    } else {
      col = bitflip_int((255 << 24) | (r << 16));
    }
  }
...
void init_mpfr(int arg0, int arg1, int arg2, int arg3, int *pixels)
{
  ...
  int bl = 255 << 24;
  mpfr_set_d(w1_init_mpfr, w / 2, MPFR_RNDZ);
  mpfr_set_d(h1_init_mpfr, h / 2, MPFR_RNDZ);
  mpfr_set_d(xe_init_mpfr, 0, MPFR_RNDZ);
  mpfr_set_d(ze_init_mpfr, 0, MPFR_RNDZ);
  mpfr_set_d(k_init_mpfr, -1, MPFR_RNDZ);
  for (y = 0; y < h; y++){
  {
    for (x = 0; x < w; x++){
    {
      ...
      mpfr_set_d(zd_init_mpfr, -1, MPFR_RNDZ);
            ...
            mpfr_add(temp_var_7, (temp_var_5), (temp_var_6), MPFR_RNDZ);

            mpfr_mul(temp_var_8, zd_init_mpfr, zd_init_mpfr, MPFR_RNDZ);
            mpfr_add(l_init_mpfr, (temp_var_7), (temp_var_8), MPFR_RNDZ);
...
//original  t_init_mpfr >= 0
)
      {
        ...
        mpfr_set(ly_init_mpfr, lly_init_mpfr, MPFR_RNDZ);
        mpfr_set_d(lz_init_mpfr, 0, MPFR_RNDZ);
                        mpfr_sub(lx_init_mpfr, lx_init_mpfr, ix_init_mpfr, MPFR_RNDZ);
        ...
        mpfr_set_d(sng_init_mpfr, (float) sqrt(((mpfr_get_d(lx_init_mpfr, MPFR_RNDZ) * mpfr_get_d(lx_init_mpfr, MPFR_RNDZ)) + (mpfr_get_d(ly_init_mpfr, MPFR_RNDZ) * mpfr_get_d(ly_init_mpfr, MPFR_RNDZ))) + (mpfr_get_d(lz_init_mpfr, MPFR_RNDZ) * mpfr_get_d(lz_init_mpfr, MPFR_RNDZ))), MPFR_RNDZ);
                ...
                mpfr_mul(temp_var_22, lz_init_mpfr, nz_init_mpfr, MPFR_RNDZ);

                mpfr_add(temp_var_23, (temp_var_21), (temp_var_22), MPFR_RNDZ);
                mpfr_mul(lcoff_init_mpfr, (temp_var_23), sng_init_mpfr, MPFR_RNDZ);
        pixels[index] = bitflip_int(texture1_bitflip(mpfr_get_d(ix_init_mpfr, MPFR_RNDZ), mpfr_get_d(iy_init_mpfr, MPFR_RNDZ), mpfr_get_d(iz_init_mpfr, MPFR_RNDZ), mpfr_get_d(lcoff_init_mpfr, MPFR_RNDZ), bitflip_int(texture), bitflip_int(light)));
      }
      else
      {
        pixels[index] = bitflip_int(bl);
      }
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < w; x++) {
      ...
      if (t >= 0) {
        ...
        sng = 1.0f / sng;
        lcoff = (lx * nx + ly * ny + lz * nz) * sng;
        pixels[index] = bitflip_int(texture1_bitflip(ix, iy, iz, lcoff, texture,light));
      } else {
        pixels[index] = bitflip_int(bl);
      }
      numIterations++;
