...
#include <mpfr.h>
    ...
    int init_mpfr() {
      ...
      mpfr_init2(val_temp, config_vals[2]);
      mpfr_init2(y_temp, config_vals[3]);
      mpfr_init2(sum_temp, config_vals[4]);
    }
    ...
    int init_readconfig() {
      ...
      // For reading precision contents of config_file into array
       FILE *myFile;
         myFile = fopen("sparse_config_file.txt", "r");
        ...
        for (reps=0; reps<NUM_ITERATIONS; reps++)
        {
            ...
            for (r=0; r<M; r++)
            {
                ...
                for (i=rowR; i<rowRp1; i++){
                    ...
                    mpfr_set_d(sum_temp,sum,MPFR_RNDZ);
                    mpfr_add(sum_temp,sum_temp,temp_variable,MPFR_RNDZ);
                    sum = mpfr_get_d(sum_temp,MPFR_RNDZ);
                  //  sum+= mpfr_get_d(temp_variable, MPFR_RNDZ);
                    //sum += x[ col[i] ] * val[i];
                  }
                mpfr_set_d(y_temp,sum,MPFR_RNDZ);
                y[r]  = mpfr_get_d(y_temp,MPFR_RNDZ);
              //  y[r] = sum;
            }
        }
